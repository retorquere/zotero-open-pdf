      ...
      try {
        ...
        if (sibling) {
          ...
          for (const att of Array.from(sibling.attributes)) {
            alternate.setAttribute(att.nodeName, att.nodeValue)
          }
          alternate.setAttribute('zotero-open-pdf-external', 'true')

          if (Zotero.Prefs.get('fileHandler.pdf')) { // existing Open option = external
            log('adding external system opener')
          ...
          for (const cmdline of (Zotero.Prefs.rootBranch.getChildList(Openers, {}, {}) as string[]).sort()) {
            ...
            for (const att of Array.from(sibling.attributes)) {
              custom.setAttribute(att.nodeName, att.nodeValue)
            }
